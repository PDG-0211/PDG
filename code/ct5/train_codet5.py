...
import logging
import torch
from mydataset import BaseDataset, ParamDesDataset, CustomizeDataset1, MultiEncoderDataset,CodeSliceDataset, ASTDataset
...
from models import BaseT5model, T5OnlyEncoderModel, MultiEncoderT5Model
...
def train(model, train_data, valid_data, args):
    ...
    for iepoch in trange(0, int(args.num_train_epochs), desc="Epoch", disable=False):
        ...
        for step, batch in enumerate(iter_bar):
            input_ids = batch["input_ids"].to(torch.device("cuda", args.device))
            attention_mask = batch["input_attention_mask"].to(torch.device("cuda", args.device))
            labels = batch["output_ids"].to(torch.device("cuda", args.device))
...
def test(model, device, tokenizer, args):
    ...
    with torch.no_grad():
        for step, batch in enumerate(test_data_loader):
            ...
            labels = batch["output_ids"].squeeze(0).to(device)
